...
* During signing, we negate the private key scalar if the public key point
  ...
  use the formula `s = k + d*e` rather than `s = k - d*e` in this case.)
  This means signatures are generated "as if" the public key had an even Y
  coordinate, which allows the signature verification to succeed with x-only
  public keys. Public keys are always treated as having an even Y coordinate
  when decoded to a point on the curve.
* Our scheme includes key-prefixing as defined in BIP 340: the public key is
  prefixed to the message in the challenge hash input. This changes the
  signature equation to `s⋅G = R + hash(r || q || m)⋅Q`.
  This protects against related-key attacks, since we make extensive use of
  key derivation (specfically BIP 32, unhardened in some cases.)
* Our scheme uses tagged hashing (as described in BIP 340) except that hashing
...
* In the signing algorithm step 3, use the following RFC6979 extra data to
  identify the scheme:
  `0xc13f0efc5ec0e84d8d7ccfbc789ae233122bdb3c7bf0fa062fb2aa7fe90a6f98`
