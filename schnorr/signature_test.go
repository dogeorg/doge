...
var testSchnorr = SchnorrImpl{
	ChallengeHasher:  blake256.NewHasher256(), // test cases assume no tagging
	RFC6979ExtraData: rfc6979ExtraDataV0[:],   // test cases assume DCRv0 scheme
}

// rfc6979ExtraDataV0 is the extra data to feed to RFC6979 when generating
// the deterministic nonce for the EC-Schnorr-DCRv0 scheme.
...
func TestSchnorrSignAndVerify(t *testing.T) {
	...
	for _, test := range tests {
		...
		if test.rfc6979 {
			...
			privKey.PutBytesUnchecked(privKeyBytes[:])
		...
		gotSig, err := testSchnorr.schnorrSign(privKey, nonce, hash, []byte{})
		if err != nil {
		...
		// Verification fails with X-only public keys because R = s*G + e*Q
		// differs in oddness of the Q.y coordinate for some of the test cases.
		// BIP 340 avoids this by negating the private key at signing time
		// when generating `s` if the derived public key has odd-Y, so the
		// resulting signature will validate against the even-Y public key.

		// This loses the sign of the Y coordinate that the Decred schnorrVerify
		// algorithm had access to. But Verification must work with just the X
		// coordinate, since that's what we use for public keys (as per BIP 340)
		pubKey, isOdd := pubKeyXfromPrivKey(privKey)
		if isOdd {
			privKey.Negate()
		}

		// Re-sign the hash of the message with the given private key and nonce,
		// but this time using the negated private key if PubKey Q.y is odd.
		// This is what our Sign() function does.
		evenSig, err := testSchnorr.schnorrSign(privKey, nonce, hash, []byte{})
		if err != nil {
			t.Errorf("%s: unexpected error when signing: %v", test.name, err)
			continue
		}
		...
		err = testSchnorr.schnorrVerify(evenSig, hash, pubKey)
		if err != nil {
...
func TestSchnorrSignAndVerifyRandom(t *testing.T) {
	...
	for i := 0; i < 10000; i++ {
		...
		overflow := privKeyScalar.SetBytes(&buf)
		...
		// private key.
		sig, err := testSchnorr.Sign(privKey, hash[:])
		if err != nil {
			t.Fatalf("failed to sign\nprivate key: %x\nhash: %x",
				privKey.Serialize(), hash)
		}
		pubKey, _ := pubKeyXfromPrivKey(&privKeyScalar)
		if !testSchnorr.Verify(sig, hash[:], pubKey) {
			t.Fatalf("failed to verify signature\nsig: %x\nhash: %x\n"+
				"private key: %x\npublic key: %x\noverflow: %v", sig.Serialize(), hash,
				privKey.Serialize(), pubKey, overflow)
		}
		...
		if testSchnorr.Verify(badSig, hash[:], pubKey) {
			t.Fatalf("verified bad signature\nsig: %x\nhash: %x\n"+
				"private key: %x\npublic key: %x\noverflow: %v", badSig.Serialize(), hash,
				privKey.Serialize(), pubKey, overflow)
		}
		...
		if testSchnorr.Verify(sig, badHash, pubKey) {
...
func TestVerifyErrors(t *testing.T) {
	tests := []struct {
		...
		hash: "c301ba9de5d6053caad9f5eb46523f007702add2c62fa39de03146a36b8026b7",
		pubX: "6b17d1f2e12c4247f8bce6e563a440f277037d812deb33a0f4a13945d898c297",
	...
	for _, test := range tests {
		...
		// The Verify API only takes the X coordinate of the PubKey (rather
		// than a fully hydrated secp256k1.PublicKey) because that's the
		// format we use to pass public keys around. This means the `pubY`
		// field in the test fixtures is ignored.
		pubX := hexToBytes(test.pubX)
		...
		err = testSchnorr.schnorrVerify(sig, hash, pubX)
		if !errors.Is(err, test.err) {
			t.Errorf("%s: mismatched err -- got %v, want %v", test.name, err,
				test.err)
			continue
...
func hexToBytes(str string) (bytes []byte) {
	bytes, err := hex.DecodeString(str)
	if err != nil {
		panic("bad hexToBytes: " + str)
	}
	return
}

func hexToModNScalar(str string) *secp256k1.ModNScalar {
	var k secp256k1.ModNScalar
	if k.SetByteSlice(hexToBytes(str)) {
		panic("bad hexToModNScalar: " + str)
	}
	return &k
}

func hexToFieldVal(str string) *secp256k1.FieldVal {
	var k secp256k1.FieldVal
	if k.SetByteSlice(hexToBytes(str)) {
		panic("bad hexToFieldVal: " + str)
	}
	return &k
}

func pubKeyXfromPrivKey(privKey *secp256k1.ModNScalar) ([]byte, bool) {
	pub := secp256k1.NewPrivateKey(privKey).PubKey().SerializeCompressed()
	isOdd := pub[0] != secp256k1.PubKeyFormatCompressedEven
	// if pub[0] != secp256k1.PubKeyFormatCompressedEven {
	// 	var pk [32]byte
	// 	privKey.PutBytes(&pk)
	// 	fmt.Printf("bad Schnorr public key: Y is odd: %v", hex.EncodeToString(pk[:]))
	// }
	return pub[1:], isOdd // X component only
}
